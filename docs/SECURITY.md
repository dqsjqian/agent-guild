...
demand by `ag upgrade`.

## Operation by operation

| Operation | Where | Why | Boundary |
|---|---|---|---|
| **HTTP request** | `_http_bytes`, `_http_json`, `fetch_*_version` | `ag upgrade` compares the installed version against the published one | Only the three public registry endpoints in `VERSION_SOURCES` and the project's own release URL. No request carries user data — no body, no query built from local state, only a `User-Agent` header. Runs only when the user invokes `ag upgrade` |
| **Downloading an archive** | `cmd_upgrade` | Install a newer version of this same skill | Only `github.com/dqsjqian/agent-guild/releases/download/...`, only under `ag upgrade --apply`, extracted into a temp dir and validated (`SKILL.md` must be present) before anything is replaced |
...
| **Temporary files** | `atomic_write_json`, `atomic_append`, `cmd_focus`, `link_capability`, `cmd_upgrade` | Atomic writes: write to a temp file in the same directory, then `os.replace`. A crash mid-write can never corrupt shared state | `tempfile.mkstemp` / `mkdtemp`, cleaned up in `finally` / `trap`. Also used to probe whether this device supports symlinks at all |
